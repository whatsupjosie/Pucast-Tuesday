...
â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
  ...
  PUBCAST_COMPUTE_MODEL     Ollama compute/Architect model tag
  PUBCAST_COMPUTE_THINK     Whether compute model may use hidden thinking
  PUBCAST_COMPUTE_KEEP_ALIVE Keep-alive for compute model; default 0 unloads it
  PUBCAST_ARCHITECT_MODEL   Optional legacy Architect GGUF file
...
_STUDIO_MODEL   = os.getenv("STUDIO_MODEL", os.getenv("OLLAMA_MODEL", "ministral-pubcast:3b"))
_OLLAMA_KEEP_ALIVE = os.getenv("OLLAMA_KEEP_ALIVE", "1h")
_COMPUTE_MODEL  = os.getenv("PUBCAST_COMPUTE_MODEL", "gemma4-compute-q5:e2b").strip()
_COMPUTE_KEEP_ALIVE = os.getenv("PUBCAST_COMPUTE_KEEP_ALIVE", "0")
_COMPUTE_THINK  = _env_bool("PUBCAST_COMPUTE_THINK", False)
...
def _load_architect() -> bool:
    ...
    if not _ARCH_PATH:
        logger.info("Orchestrator: PUBCAST_ARCHITECT_MODEL not set â€” Architect disabled")
        return False
...
class LLMOrchestrator:
    ...
    def __init__(self) -> None:
        ...
        self._compute_model = _COMPUTE_MODEL
        self._compute_keep_alive = _COMPUTE_KEEP_ALIVE
        self._compute_think = _COMPUTE_THINK
        ...
        self._arch_ok     = False
        self._last_arch_model = self._compute_model or _ARCH_PATH
    ...
    def _refresh_policy(self) -> None:
        ...
        compute_model = str(perf.get("compute_model", self._compute_model) or self._compute_model)
    ...
    async def startup(self) -> None:
        ...
        # Architect/compute probe. Ollama compute is preferred; legacy GGUF stays available.
        gguf_ok = _load_architect()
        self._arch_ok = bool(self._compute_model and self._studio_ok) or gguf_ok
        logger.info(
            "Orchestrator: Architect â€” %s  (path: %s)",
            "AVAILABLE" if self._arch_ok else "UNAVAILABLE",
            self._compute_model or _ARCH_PATH or "not configured",
        )
    ...
    async def _ollama_compute_call(
        self,
        prompt: str,
        ctx: ContextPacket,
        temperature: float,
        ...
        """Run the Ollama compute model as the Architect mind."""
        if not self._compute_model:
            return None

        messages: List[Dict[str, str]] = []
        sys_prompt = ctx.architect_system()
        if sys_prompt:
            messages.append({"role": "system", "content": sys_prompt})
        messages.extend(ctx.history_as_messages())
        messages.append({"role": "user", "content": prompt})
        ...
        async with httpx.AsyncClient(timeout=float(os.getenv("OLLAMA_TIMEOUT", "180"))) as c:
            r = await c.post(
                ...
                json={
                    ...
                    "messages": messages,
                    "stream": False,
                    "keep_alive": self._compute_keep_alive,
                    "think": self._compute_think,
                    "options": {
                        "temperature": temperature,
                        "num_ctx": 2048,
    ...
    async def _architect_call(
        ...
        if self._breaker.is_open:
            return None
        if not self._compute_model and (not _arch_available or _arch_llm is None):
            return None
            ...
            try:
                if self._compute_model:
                    text = await asyncio.wait_for(
                        self._ollama_compute_call(prompt, ctx, temperature, max_tokens),
                        timeout=self._architect_timeout_s,
                    )
                    if not text:
                        self._breaker.record_failure()
                        return None
                    self._last_arch_model = self._compute_model
                else:
                    text = await asyncio.wait_for(
                    ...
                    self._last_arch_model = _ARCH_PATH
    ...
    def status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "architect": {
                ...
                "provider":    "ollama" if self._compute_model else "gguf",
                "compute_model": self._compute_model or None,
